# Spyridon C. Katsoudas, Mathematician. PhD Candidate 

I am a computational researcher focusing on the intersection of **Fluid Dynamics**, **Machine Learning**, and **Cardiovascular Pathology**. My work bridges the gap between rigorous applied mathematics and clinical translation, aiming to accelerate high-fidelity Computational Fluid Dynamics (CFD) simulations to better understand complex blood flow patterns, turbulence, and arterial disease.

---

### 🔬 Research Focus & Experience
* **Computational Hemodynamics:** Multiscale modeling and topological fluid dynamics for Abdominal Aortic Aneurysms (AAAs) and Endovascular Aneurysm Repair (EVAR).
* **Applied Mathematics in Fluid Mechanics:** Developing fast, robust AI frameworks (Physics-Informed ML & Surrogates) to bypass computationally expensive RANS/SST k-ω simulations in stenotic and bifurcating arteries.

---

### 📄 Selected Publications
* **Linking aneurysmal geometry and hemodynamics using computational fluid dynamics**
  *Physics of Fluids (2026)*
* **Hemodynamic Shear Stress Patterns in Abdominal Aortic Aneurysms and Healthy Aortas: A CFD Study**
  *Cardiovascular Engineering and Technology (2026)*
* **Comparison of Hemodynamics After Fenestrated, Branched, and Chimney Endovascular Aneurysm Repair Employing CFD**
  *Journal of Clinical Medicine (2026)*
* **Accelerating the Simulation of Pulsatile Turbulent Hemodynamics in Stenotic Arteries using Artificial Neural Networks**
  *Meccanica*

*(See my Google Scholar for the complete list of publications and book chapters).*

---

### 💻 Tech Stack & Tools
* **Programming:** Python, MATLAB, Mathematica
* **CFD & Meshing:** ANSYS Fluent, SimVascular, OpenFOAM
* **Post-processing & 3D:** ParaView, Meshmixer, Tecplot
* **Methods:** Finite Volume Methods (FVM), Turbulence Modeling, Optimization (Levenberg-Marquardt), Supervised Machine Learning

---

### 📫 Let's Connect
* **Google Scholar:** [Spyridon Katsoudas](https://scholar.google.com/citations?user=XoU7ruMAAAAJ&hl=el)
* **ResearchGate:** [Spyridon-Katsoudas](https://www.researchgate.net/profile/Spyridon-Katsoudas)
* **ORCID:** [0009-0006-7700-7261](https://orcid.org/0009-0006-7700-7261)
* **Email:** s.katsoudas@uoi.gr

> *"Advancing cardiovascular care through computational speed and mathematical rigor."*
